    ...
    return (
        <div className="flex flex-col h-[calc(100vh-140px)] bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden animate-fade-in">
            <div className="bg-[#17B8A5] p-4 flex items-center gap-3 text-white shadow-sm">
                <div className="p-2 bg-white/20 rounded-full"><Bot size={24}/></div><div><h3 className="font-bold">Asisten HR</h3><p className="text-xs opacity-90">Online • AI Support</p></div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-900">
                {messages.map(msg => (
                    <div key={msg.id} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                        <div className={`max-w-[80%] p-3.5 rounded-2xl text-sm ${msg.sender === 'user' ? 'bg-[#17B8A5] text-white rounded-tr-none' : 'bg-white dark:bg-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-600 rounded-tl-none shadow-sm'}`}>{msg.text}</div>
                        {msg.data && (
                            <div className="mt-2 w-[80%] bg-white dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm">
                                <div className="max-h-48 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-600">
                                    {msg.data.map((d, i) => (<div key={i} className="p-3 text-sm hover:bg-gray-50 dark:hover:bg-gray-600"><p className="font-bold text-gray-800 dark:text-gray-200">{d.title}</p><p className="text-xs text-gray-500 dark:text-gray-400">{d.desc}</p></div>))}
                                </div>
                            </div>
                        )}
                        {msg.quickReplies && (<div className="flex flex-wrap gap-2 mt-2">{msg.quickReplies.map(r => (<button key={r} onClick={() => handleSend(r)} className="text-xs px-3 py-1.5 bg-white dark:bg-gray-700 border border-[#17B8A5] text-[#17B8A5] rounded-full hover:bg-[#F0FDFA] transition">{r}</button>))}</div>)}
                    </div>
                ))}
